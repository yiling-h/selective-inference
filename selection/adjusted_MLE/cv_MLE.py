...
def compare_split_MLE(n=500, p=100, nval=500, rho=0.35, s=5, beta_type=1, snr=0.20, target= "selected",
                      full_dispersion=True, tuning_rand="lambda.1se", tuning_nonrand="lambda.1se",
    ...
    lam_theory = sigma_ * 1. * np.mean(np.fabs(np.dot(X.T, np.random.standard_normal((n, 2000)))).max(0))
    _, _, _, lam_min, lam_1se = glmnet_lasso(X, y, lam_theory / float(n))
    randomization_cov = ((sigma_ ** 2) * ((1. - split_proportion) / split_proportion)) * Sigma
    ...
    if tuning_nonrand == "lambda.min":
        glm_LASSO_split, _, _, _, _ = glmnet_lasso(X_sel, y_sel, lam_min)
    elif tuning_nonrand == "lambda.1se":
        glm_LASSO_split, _, _, _, _ = glmnet_lasso(X_sel, y_sel, lam_1se)
    else:
        glm_LASSO_split, _, _, _, _ = glmnet_lasso(X_sel, y_sel, lam_theory/ float(subsample_size))
...
# compare_split_MLE(n=500, p=100, nval=500, rho=0.35, s=5, beta_type=1, snr=0.20, target= "selected",
#                   full_dispersion=True, tuning_rand="lambda.1se",tuning_nonrand="lambda.1se",
