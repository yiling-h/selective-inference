...
def nonnormal_instance(n=100, p=200, s=7, sigma=5, rho=0., signal=7,
                    random_signs=False, df=np.inf,
                    scale=True, center=True,
                    equicorrelated=True):
...
def mixed_normal_instance(n=100, p=200, s=7, sigma=5, rho=0., signal=7,
                    random_signs=False, df=np.inf,
                    scale=True, center=True,
                    equicorrelated=True):
    ...
    if random_signs:
        beta[:s] *= (2 * np.random.binomial(1, 0.5, size=(s,)) - 1.)
    np.random.shuffle(beta)
    beta /= (np.sqrt(n)/np.power(n, -0.33))
