...
def gaussian_instance(n=100, p=200, s=7, sigma=5, rho=0., signal=7,
    ...
    beta /= (np.sqrt(n) / np.power(n, -1. / 6. + 0.0001))
...
def exp_instance(n=100, p=200, s=7, sigma=5, rho=0., signal=7,
    ...
    Y = (X.dot(beta) + _noise(n)) * sigma
    return X, Y, beta * sigma, np.nonzero(active)[0], sigma, sigmaX

def normexp_instance(n=100, p=200, s=7, sigma=5, rho=0., signal=7,
                     random_signs=False, scale=True, center=True,
                     equicorrelated=True):
    ...
    def _noise(n):
        return (np.random.standard_normal(n) + np.random.exponential(scale=1.0, size=n) - 1.)/np.sqrt(2.)

    Y = (X.dot(beta) + _noise(n)) * sigma
    return X, Y, beta * sigma, np.nonzero(active)[0], sigma, sigmaX

def mixednormal_instance(n=100, p=200, s=7, sigma=5, rho=0., signal=7,
                         random_signs=False,  scale=True, center=True,
                         equicorrelated=True):
    ...
    # noise model
    def _noise(n):
        mix_error = np.zeros(n)
        error = np.random.uniform(0., 1., n)
        for k in range(n):
            if error[k] <= 0.8:
                mix_error[k] = np.random.normal(loc=0.0, scale= np.sqrt(0.25), size=1)
            else:
                mix_error[k] = np.random.normal(loc=0.0, scale= np.sqrt(3.), size=1)
        return mix_error/np.sqrt(4./5.)

    Y = (X.dot(beta) + _noise(n)) * sigma
    return X, Y, beta * sigma, np.nonzero(active)[0], sigma, sigmaX

def laplace_instance(n=100, p=200, s=7, sigma=5, rho=0., signal=7,
                     random_signs=False, scale=True, center=True,
                     equicorrelated=True):
