...
def grp_lasso_selection_std(X, Y, traj, randomize=True):
    # standardized group lasso; assume contiguous groups
...
def naive_inference(traj, X, Y, beta):
    ...
    nonzero, conv, _ = grp_lasso_selection_std(X, Y, traj, randomize=False)
...
def posi_std(traj, X, Y, beta):
    nonzero, conv, dispersion = grp_lasso_selection_std(X, Y, traj, randomize=True)
...
def data_splitting(traj, X, Y, beta):
    ...
    nonzero, conv, _ = grp_lasso_selection_std(X_train, Y_train, traj, randomize=False)
