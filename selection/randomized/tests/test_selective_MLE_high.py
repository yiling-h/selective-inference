...
from selection.tests.instance import gaussian_instance, _design
...
def test_selected_targets(n=2000, 
    ...
    while True:
        ...
        sigma_ = np.std(Y)
        #sigma_ = sigma
        W = 0.85* np.ones(X.shape[1]) * np.sqrt(2 * np.log(p)) * sigma_
        ...
        signs = conv.fit()
        nonzero = signs != 0
        print("dimensions", n, p, nonzero.sum(), full_dispersion)
        print("check ", sigma_, sigma)
...
def main(nsim=500, full=False):
    ...
    for i in range(nsim):
        ...
        P0.extend(p0)
        PA.extend(pA)
        print(np.array(PA) < 0.1, np.mean(P0), np.std(P0), np.mean(np.array(P0) < 0.1), np.mean(np.array(PA) < 0.1), np.mean(cover),
