...
if __name__ == "__main__":
    ...
    for egene in range(len(content)):
        ...
        unad_risk = np.power(point_est, 2.).sum() / nactive
        coverage_unad = np.zeros(nactive)
        
        for k in range(true_val.shape[0]):
            if (unad_intervals[k, 0] <= 0.) and (0. <= unad_intervals[k, 1]):
                coverage_unad[k] += 1
        coverage_unad = coverage_unad.sum() /float(nactive)
        nsig = 1. - coverage_unad
        output = np.vstack((nactive, coverage_unad, unad_length, unad_risk, nsig))
