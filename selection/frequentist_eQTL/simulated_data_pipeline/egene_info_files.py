...
if __name__ == "__main__":
    ...
    for i in range(len(content)):
        ...
        y = np.load(os.path.join(Y_path + "y_" + str(content[i])) + ".npy")
        y = y.reshape((y.shape[0],))
        out_Y = os.path.join(outdir + "y_" + str(content[i]) + ".npy")
        np.save(out_Y, y)
        
        beta = np.load(os.path.join(Y_path + "b_" + str(content[i])) + ".npy")
        beta = beta.reshape((beta.shape[0],))
        out_beta = os.path.join(outdir + "b_" + str(content[i]) + ".npy")
        np.save(out_beta, beta)

        prototypes = np.loadtxt(os.path.join(proto_path + "protoclust_"  + str(content[i])) + ".txt", delimiter='\t')
