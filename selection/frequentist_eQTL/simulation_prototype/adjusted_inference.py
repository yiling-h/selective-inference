...
class selection_map2step(M_estimator):
    ...
    def solve_approx(self):
        ...
        self.score_cov_simes = self.sigma_ratio * (X_active_inv.dot(X[:, self._overall].T).dot(X[:, self.index]))
...
def hierarchical_lasso_trial(X,
    ...
    if nactive == 0:
        ...
        try:
            ...
            sys.stderr.write("True target to be covered" + str(true_vec) + "\n")
            sys.stderr.write("Total adjusted covered" + str(sel_covered.sum()/float(nactive)) + "\n")
            sys.stderr.write("Total naive covered" + str(naive_covered.sum()/float(nactive)) + "\n")
