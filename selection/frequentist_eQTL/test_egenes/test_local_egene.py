...
from selection.frequentist_eQTL.test_egenes.inference_hierarchical_selection import M_estimator_2step, approximate_conditional_density_2stage
...
if __name__ == "__main__":
    ...
    gene = str("ENSG00000132881.7")
    #X = np.load(os.path.join(path + "X_" + "ENSG00000237094.7") + ".npy")
    #X = np.load(os.path.join(path + "X_" + "ENSG00000242349.1") + ".npy")
    ...
    #prototypes = np.loadtxt("/Users/snigdhapanigrahi/Test_egenes/Egene_data/protoclust_ENSG00000237094.7.txt", delimiter='\t')
    #prototypes = np.loadtxt("/Users/snigdhapanigrahi/Test_egenes/Egene_data/protoclust_ENSG00000242349.1.txt",
    #                        delimiter='\t')
    ...
    #y = np.load(os.path.join(path + "y_" + "ENSG00000237094.7") + ".npy")
    #y = np.load(os.path.join(path + "y_" + "ENSG00000242349.1") + ".npy")
    ...
    #simes_output = np.loadtxt("/Users/snigdhapanigrahi/Test_egenes/Egene_data/simes_ENSG00000237094.7.txt")
    #simes_output = np.loadtxt("/Users/snigdhapanigrahi/Test_egenes/Egene_data/simes_ENSG00000242349.1.txt")
    simes_output = np.loadtxt(os.path.join("/Users/snigdhapanigrahi/Test_egenes/Egene_data/simes_" + gene) + ".txt")

    simes_level = (0.10 * 2310)/21819.
    index = int(simes_output[3])
    T_sign = simes_output[7]
    ...
    u_2 = simes_output[5]
    u_3 = simes_output[6]
    sigma_hat = simes_output[8]

    if i_0 <= p - 2 and u_1 !=0 and u_2 !=0:
        l_threshold = np.sqrt(1+ (0.7**2)) * normal.ppf(1. - min(u_1, u_2, simes_level * ((i_0 + 1.) / V)) / 2.)
    else:
        l_threshold = 10 ** 10

    if u_3 != -1:
        u_threshold = np.sqrt(2.) * normal.ppf(1. - u_3 / 2.)
    else:
        u_threshold = 10 ** 10

    u_threshold = 10 ** 10

    print("u_1, u_2, u_3, i_0", u_1, u_2, u_3, i_0)
    print("l and u threshold", l_threshold, u_threshold)
