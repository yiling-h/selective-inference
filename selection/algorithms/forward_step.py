...
class forward_step(object):
    ...
    def step(self,
        ...
        self.maxZ_offset[0] = np.minimum(self.maxZ_offset[0],
                                         realized_Z_adjustment + fit_adjustment)  # (Z_max + V) * S_L
        self.maxZ_offset[1] = np.minimum(self.maxZ_offset[1],
                                         realized_Z_adjustment - fit_adjustment)  # (Z_max - V) * S_L

        # update our list of variables and signs

        self.inactive[winning_var] = False  # inactive is now losing_vars
        self.variables.append(winning_var);
        self.signs.append(winning_sign)
