...
class group_lasso(gaussian_query):
    ...
    def fit(self,
        ...
        if self.overdispersed:
            mu_hat = np.exp(X @ soln)  # Fitted values
            overdispersion = 1 / (n - p) * ((y - mu_hat) ** 2).T @ (1 / mu_hat)
            self.overdispersion = overdispersion

        opt_linearNoU = np.dot(X.T, X[:, ordered_vars] * W[:, np.newaxis]) / self.overdispersion

        ## TODO: Adjust for the ridge term
        for i, var in enumerate(ordered_vars):
            opt_linearNoU[var, i] += self.ridge_term

        self.observed_score_state = -opt_linearNoU.dot(_beta_unpenalized)
        self.observed_score_state[~overall] += \
            (self.loglike.smooth_objective(beta_bar, 'grad')[~overall] / self.overdispersion)
        ...
        ## Estimating the covariance matrix of the score vector
    ...
    def quasipoisson(X,
        ...
        if overdispersion is None:
            if n > p:
                # Solving the unrestricted MLE
                def solve_MLE():
                    loglike = rr.glm.poisson(X, counts=counts)
                ...
                overdispersion = 1/(n-p) * ((counts - mu_hat)**2).T @ (1/mu_hat)
                print("overdispersion parameter:", overdispersion)
            else:
                overdispersion = None

        loglike = rr.glm.poisson(X, counts, quadratic=quadratic, overdispersion = overdispersion)
