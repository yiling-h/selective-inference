...
def randomization_inference_fast(X, Y, n, p, beta, groups, proportion, hess=None,
                                 weight_frac=1, level=0.9, solve_only=False):
    ## solve_only: bool variable indicating whether
    ##              1) we only need the solver's output
    ##              or
...
def test_comparison_poisson_group_lasso(n=500,
    ...
    oper_char_df.to_csv('selectinf/randomized/tests/pois_vary_signal.csv', index=False)
    colnames = ['Index'] + ['target'] + ['LCB'] + ['UCB'] + ['TP'] + ['beta size'] + ['F1'] + ['Method']
    confint_df.columns = colnames
    confint_df.to_csv('selectinf/randomized/tests/pois_CI_vary_signal.csv', index=False)
...
def test_plotting_separate(path='selectinf/randomized/tests/pois_vary_sparsity.csv'):
    oper_char_df = pd.read_csv(path)
