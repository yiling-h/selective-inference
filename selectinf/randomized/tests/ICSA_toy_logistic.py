...
def comparison_logistic_lasso_vary_s(n=500,
        ...
        for i in range:
            ...
            while True:  # run until we get some selection
                ...
                X, Y, beta = inst(n=n,
                                  ...
                                  ndiscrete=5,
                                  nlevels=5,
                                  sdiscrete=s,  # s-3, # How many discrete rvs are not null
                                  equicorrelated=False,
                                  rho=rho,
                                  random_signs=True)[:3]
                # print(X)
    ...
    oper_char_df = pd.DataFrame.from_dict(oper_char)
    oper_char_df.to_csv('logis_ICSA_toy.csv', index=False)
    colnames = ['Index'] + ['target'] + ['LCB'] + ['UCB'] + ['TP'] + ['sparsity size'] + ['F1'] + ['Method']
    confint_df.columns = colnames
    confint_df.to_csv('logis_CI_ICSA_toy' + str(range.start) + '_' + str(range.stop) + '.csv', index=False)
...
if __name__ == '__main__':
    ...
    comparison_logistic_lasso_vary_s(range=range(0,1000))
